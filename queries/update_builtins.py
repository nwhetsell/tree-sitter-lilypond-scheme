...
with open('highlights-builtins.scm', 'w') as file:
    regex = re.sub(r'\\(.)', r'\\\\\1', regex_opt(_scheme_builtins.scheme_keywords, '^', '$'))
    file.write(dedent(f'''\
    (
    ...
    regex = re.sub(r'\\(.)', r'\\\\\1', regex_opt(_scheme_builtins.scheme_builtins, '^', '$'))
    file.write(dedent(f'''\
    (
...
with open('highlights-lilypond-builtins.scm', 'w') as file:
    regex = re.sub(r'\\(.)', r'\\\\\1', regex_opt(_lilypond_builtins.scheme_functions, '^', '$'))
    file.write(dedent(f'''\
    (
      (scheme_symbol) @identifier.core.function
      (#match? @identifier.core.function "{regex}")
